    ...
    <div class="row">
        <div class="col-xs-12 col-md-8">
            ...
            <div class="form-group"><label
                        class="col-sm-2 control-label">{!! Form::label('active','Active:') !!}</label>
                <div class="col-sm-10">
                    <input type="checkbox" name="active" value="1"
                           class="js-switch" {{(isset($post->active) && $post->active == 1?'checked':'')}} />
                </div>
            </div>
            ...
            <div class="form-group"><label class="col-sm-2 control-label">Summary</label>
                <div class="col-sm-10">
                    <textarea class="form-control"
                              name="summary">{{(isset($post->summary)?$post->summary:'')}}</textarea>
                </div>
            </div>
            <div class="hr-line-dashed"></div>
            ...
            <div class="form-group hidden"><label class="col-sm-2 control-label">Images</label>
                <div class="col-sm-10">
                    {!! Form::file('image[]',['multiple' => "multiple" ]) !!}
                    @if(isset($post->image))
                        @foreach($post->image as $img)
                            <img data-id="{{$img->id}}"
                                 class="thumb_img {{($post->main_img == $img->id?'active_box':'')}}"
                                 src="/{{$img->url}}" style="width:100px;" data-id="{{$img->id}}"
                                 data-pid="{{$post->id}}">
                        @endforeach
                    @endif
            ...
            <div class="hr-line-dashed"></div>
            <div class="form-group"><label class="col-sm-2 control-label">Groups</label>
                <div class="col-sm-10">
                    @if(isset($groups))
                        @include('admin.includes.groups.show-box-group-admin',['groups'=>$groups,"checked_group"=>$checked_group])
                    @endif
                </div>
            </div>
...
@section('custom_footer')
    ...
    <script>
        ...
        $(".thumb_img").click(function () {
            ...
            $.post('/admin/update_ajax', {
                'process': 'change_main_img',
                'id': $(this).data('id'),
                'pid': $(this).data('pid')
            }, function (data) {
